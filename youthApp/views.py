...
<<<<<<< HEAD
                ...
                html_user = render_to_string('email/applicant_confirmation.html', context_user)
                text_user = strip_tags(html_user)

                email_user = EmailMultiAlternatives(
                    subject=f"Thanks for applying: {job.job_title}",
                    body=text_user,
                ...
                html_company = render_to_string('email/company_notification.html', context_company)
                text_company = strip_tags(html_company)
                ...
                email_company.attach_alternative(html_company, "text/html")
                email_company.send()
...
def enroll_course(request, course_id):
    ...
    if existing_enrollment:
        messages.success(request, "Waxaad horey u bixisay koorsadan. Waxaad si toos ah u heli kartaa casharrada.")
        return redirect('course_lessons', course_id=course.id)  # ama view ku habboon
    ...
    if request.method == 'POST':
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            mode='payment',
            success_url=request.build_absolute_uri(reverse('payment_success', kwargs={'course_id': course.id})),
            cancel_url=request.build_absolute_uri(reverse('payment_cancel')),
        )
        return redirect(checkout_session.url, code=303)

    return render(request, 'user/course_page/enroll_confirm.html', {'course': course})





# ➤ View: Marka lacag bixintu guuleysato
@login_required
def payment_success(request, course_id):
    course = get_object_or_404(TrainingCourse, id=course_id)
    
    # Check if already exists before creating a new one
    ...
    messages.success(request, "Lacag bixinta waa la dhammeystiray! Waxaad hadda heli kartaa casharrada.")
    return redirect('course_lessons', course_id=course.id)



# ➤ View: Haddii lacag la diido
@login_required
def payment_cancel(request):
    messages.error(request, "Lacag bixintu waa la joojiyey.")
    return redirect('all_courses')


# ➤ View: Casharrada koorsada (kaliya kuwa is diiwaangeliyay & bixiyay)
...
def course_lessons(request, course_id):
    try:
        ...
        raise Http404("Koorsada lama helin")

    enrollment = Enrollment.objects.filter(user=request.user, course=course, is_paid=True).exists()
